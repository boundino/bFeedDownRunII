...
void projectPPMBMC(TString col)
{
  ...
  TCut cutPPMb = Form("(1)&&%s",CUTPPMB.Data());
  TCut cutpt = cutptMB;
  TCut weightfunctionreco = weightfunctionrecoPPMB;
  ...
  for(int i=0;i<nwidth;i++)
    {
      ...
      tMCP->Draw(Form("DsvpvDistance/DsvpvDisErr:(DsvpvDistance*sin(Dalpha))*(1-%f):Dpt>>hMCPSignal_%d",width,i),weightfunctionreco*(cutpt&&cutPPMb&&cutmc&&cutPrompt));
      tMCNP->Draw(Form("DsvpvDistance/DsvpvDisErr:(DsvpvDistance*sin(Dalpha))*(1-%f):Dpt>>hMCNPSignal_%d",width,i),weightfunctionreco*(cutpt&&cutPPMb&&cutmc&&cutNonPrompt));
      tMCP->Draw(Form("(DsvpvDistance*sin(Dalpha))*(1-%f):Dmass:Dpt>>hPtMD0DcaMCPSignal_%d",width,i),cutpt&&cutPPMb&&cutmcSignal);
      tMCP->Draw(Form("(DsvpvDistance*sin(Dalpha))*(1-%f):Dmass:Dpt>>hPtMD0DcaMCPSwapped_%d",width,i),cutpt&&cutPPMb&&cutmcSwapped);
    }
