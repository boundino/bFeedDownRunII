...
void projectPbPbMBMC(TString col, Float_t centmin=0, Float_t centmax=100)
{
  ...
  for(int i=0;i<nwidth;i++)
    {
      ...
      tMCP->Draw(Form("DsvpvDistance/DsvpvDisErr:(DsvpvDistance*sin(Dalpha))*(1-%f):Dpt>>hMCPSignal_%d",width,i),weightfunctionreco*(cutpt&&cutPbPbMB&&cutmc&&cutPrompt));
      tMCNP->Draw(Form("DsvpvDistance/DsvpvDisErr:(DsvpvDistance*sin(Dalpha))*(1-%f):Dpt>>hMCNPSignal_%d",width,i),weightfunctionreco*(cutpt&&cutPbPbMB&&cutmc&&cutNonPrompt));
      tMCP->Draw(Form("(DsvpvDistance*sin(Dalpha))*(1-%f):Dmass:Dpt>>hPtMD0DcaMCPSignal_%d",width,i),cutpt&&cutPbPbMB&&cutmcSignal);
      tMCP->Draw(Form("(DsvpvDistance*sin(Dalpha))*(1-%f):Dmass:Dpt>>hPtMD0DcaMCPSwapped_%d",width,i),cutpt&&cutPbPbMB&&cutmcSwapped);
    }
