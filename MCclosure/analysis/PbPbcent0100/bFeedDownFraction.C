...
void bFeedDownFraction()
{
  ...
  TFile* fPbPb = new TFile("rootfiles/bFeedDownPbPb.hist.root");
  TFile* fPbPbMB = new TFile("rootfiles/bFeedDownPbPbMB.hist.root");
  TFile* fPbPbMC = new TFile("rootfiles/bFeedDownPbPbMC.hist.root");
  TFile* fPbPbMBMC = new TFile("rootfiles/bFeedDownPbPbMBMC.hist.root");
  ...
  float pts[nPtBins];
  float mcPrompt[nPtBins];
  float mcNonPrompt[nPtBins];
  ...
  TH1D** ahD0DcaMCPSignalreal = new TH1D*[nPtBins];
  TH1D** ahD0DcaMCNPSignalreal = new TH1D*[nPtBins];
  ...
  for(int i=1; i<=nPtBins; i++)
      ...
      for(int j=1; j<=13; j++)
	{
      ...
      ahD0DcaMCPSignalreal[i-1] = (TH1D*)hD0DcaMCPSignal->Clone(Form("hD0DcaMCPSignalreal_%d",i-1));
      ...
      ahD0DcaMCNPSignalreal[i-1] = (TH1D*)hD0DcaMCNPSignal->Clone(Form("hD0DcaMCNPSignalreal_%d",i-1));
      ...
      while(fitStatus)
	{
      ...
      mcPrompt[i-1] = fMix->GetParameter(0)*fMix->GetParameter(1);
      mcNonPrompt[i-1] = fMix->GetParameter(0)*(1-fMix->GetParameter(1));
  ...
  TH1D* hFraction = new TH1D("hFraction","",nPtBins,binsX);
  TH1D* hPrompt = new TH1D("hPrompt","",nPtBins,binsX);
  TH1D* hNonPrompt = new TH1D("hNonPrompt","",nPtBins,binsX);
  for(int i=0;i<nPtBins;i++)
    {
      hPrompt->SetBinContent(i+1,mcPrompt[i]);
      hNonPrompt->SetBinContent(i+1,mcNonPrompt[i]);
      hFraction->SetBinContent(i+1,promptFraction[i]);
      hFraction->SetBinError(i+1,promptFractionErrorDataOnly[i]);
    }
  ...
  c1->SetLogy();
  float ptBins[nPtBins+1] = {2.,4.,5.,6.,8.,10.,12.5,15.0,20.,25.,30.,40.,60.,100};
  ...
  for(int i=0;i<nPtBins;i++) ahD0DcaMCPSignalreal[i]->Write();
  for(int i=0;i<nPtBins;i++) ahD0DcaMCNPSignalreal[i]->Write();
  /*
